...
document.addEventListener("deviceready",function(){
	$("#boton1").on("tap", function (){
		...
		$.ajax({
			...
			{
				$('#contenidopro').append('<div><div><div style="display:inline-block"><img src="http://192.168.1.192/ferreteria/recursos/imagenes/fotos/'+datosProductos.productos[x].claveProducto+'.jpg"></div><div style="display:inline-block"><h1>'+ datosProductos.productos[x].nombreProducto +' </h1><h2> '+datosProductos.productos[x].marcaProducto+' </h2></div></div><div><h2> '+datosProductos.productos[x].precioUnitario+' </h2><h2> '+datosProductos.productos[x].precioMayoreo+' </h2></div></div>');
			}
