...
document.addEventListener("deviceready",function(){
	$("#boton1").on("tap", function (){
		$.ajax({
			...
			{
				$('#contenido').append('<div><div><div style="display:inline-block"><img src="http://192.168.1.68/ferreteria/recursos/imagenes/fotos/'+datosProducto.productos[x].claveProducto+'.jpg"></div><div style="display:inline-block"><h1>'+ datosProducto.productos[x].nombreProducto +' </h1><h2> '+datosProducto.productos[x].marcaProducto+' </h2></div></div><div><h2> '+datosProducto.productos[x].precioUnitario+' </h2><h2> '+datosProducto.productos[x].precioMayoreo+' </h2></div></div>');
			}
		...
		$.ajax({
			...
			{
				$('#contenidopro').append('<div><div><div style="display:inline-block"><img src="http://192.168.1.68/ferreteria/recursos/imagenes/fotos/'+datosProductos.productos[x].claveProducto+'.jpg"></div><div style="display:inline-block"><h1>'+ datosProductos.productos[x].nombreProducto +' </h1><h2> '+datosProductos.productos[x].marcaProducto+' </h2></div></div><div><h2> '+datosProductos.productos[x].precioUnitario+' </h2><h2> '+datosProductos.productos[x].precioMayoreo+' </h2></div></div>');
			}
		...
		$.ajax({
			...
			{
				$('#contenidomar').append('<div><div><div style="display:inline-block"><img src="http://192.168.1.68/ferreteria/recursos/imagenes/fotos/'+datosMarca.marcas[x].claveProducto+'.jpg"></div><div style="display:inline-block"><h1>'+ datosMarca.marcas[x].nombreProducto +' </h1><h2> '+datosMarca.marcas[x].marcaProducto+' </h2></div></div><div><h2> '+datosMarca.marcas[x].precioUnitario+' </h2><h2> '+datosMarca.marcas[x].precioMayoreo+' </h2></div></div>');
			}
